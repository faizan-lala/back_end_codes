    ...
    static fetchProductById(productid){
      ...
      update(){
        return new Promise((resolve,reject)=>{
         pool.getConnection((err,con)=>{
           if(!err){
             let sql = "update product set productName=?,productPrice=?,productQty=?,description=?,frontview=?,backview=?,leftview=?,rightview=? where productid= ?";
             con.query(sql,[this.productName,this.productPrice*1,this.productQty,this.description,this.frontview,this.backview,this.leftview,this.rightview,this.productid],(err,result)=>{
               err ? reject(err) : resolve(result);
               con.release();
             });
           }
